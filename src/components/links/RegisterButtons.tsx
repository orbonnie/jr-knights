"use client";

import { useHoverReset } from "@/hooks/useHoverReset";

const MIDDLE_REGISTRATION_URL = "https://registration.teamsnap.com/form/51390";
const ELEMENTARY_REGISTRATION_URL =
  "https://cobbfootball.leagueapps.com/clubteams/5052028-2026-centennial-youth-football";

export default function RegisterButtons({
  label = "Middle School Registration",
  hover = "bg-black-500 text-white",
}: {
  label?: string;
  hover?: string;
}) {
  const k5RegHover = useHoverReset();
  const msRegHover = useHoverReset();

  const regButtons = [
    {
      label: label,
      href: MIDDLE_REGISTRATION_URL,
      hook: msRegHover,
    },
    {
      label: "K–5th Registration",
      href: ELEMENTARY_REGISTRATION_URL,
      hook: k5RegHover,
    },
  ];

  return (
    <div className="flex flex-row gap-2 sm:gap-4 items-stretch">
      {regButtons.map((button) => (
        <a
          key={button.label}
          href={button.href}
          target="_blank"
          rel="noopener noreferrer"
          onMouseEnter={button.hook.onMouseEnter}
          onMouseLeave={button.hook.onMouseLeave}
          className={`font-display tracking-widest uppercase px-3 py-2 text-xs sm:px-8 sm:py-4 sm:text-base flex-1 sm:w-64 rounded-lg sm:rounded-xl transition-all duration-200 text-center flex items-center justify-center ${
            button.hook.hovered ? hover : "bg-royal-600 text-white"
          }`}
        >
          {button.label}
        </a>
      ))}
    </div>
  );
}
